    ...
    T: serde::de::DeserializeOwned,
{
    ...
    tracing::debug!("loading defaults from {}", defaults_path.display());
    let mut merged = json_loader::load_validated(&defaults_path, &schema_path)
        .map_err(|e| map_json_error(e, &defaults_path))?;
    tracing::debug!("defaults loaded from {}: {merged}", defaults_path.display());

    // 3: try user override.
    if let Some(user_path) = user_path_fn() {
        if user_path.exists() {
            tracing::info!("user override found at {}", user_path.display());
            let user_value =
                json_loader::read_json(&user_path).map_err(|e| map_json_error(e, &user_path))?;
            tracing::debug!("user override content: {user_value}");
            ...
            tracing::info!("user override for {name} validated and merged");
        } else {
            tracing::debug!(
    ...
    // 4: deserialise.
    tracing::info!("final {name} config after defaults + overrides: {merged}");
