...
- **Camera, render layer configuration, and UI rendering infrastructure are exempt**: Setting `RenderLayers`, camera `order`, `UiSourceCamera`, and similar camera/render infrastructure in Rust code is not visual content creation. Small procedural textures for UI rendering (e.g., gradient overlays for scroll fade effects) are also exempt. These are plumbing concerns required by Bevy's rendering architecture and are not visual content like game textures, meshes, shaders, or materials.
- **UI rendering elements are exempt**: Procedural generation of UI elements (gauge scales, needles, arrow indicators, progress bars, etc.) is exempt from this rule. These are UI rendering infrastructure, not "objects in space" like ships, planets, or stations. The external JSON files describe WHERE these elements are placed (e.g., slot positions in cockpit stations), but the visual rendering of those elements (scales, needles, fills) is handled procedurally in Rust as part of the UI rendering pipeline.
