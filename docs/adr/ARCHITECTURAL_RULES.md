...
- ❌ No visual data generation or handling in Rust code (textures, meshes, shaders, render targets, materials, animations) **except**: (1) debug/diagnostic visualizations, (2) camera setup, render layer configuration, and UI rendering infrastructure (`RenderLayers`, camera `order`, `UiSourceCamera`, procedural gradient textures for scroll fade effects), (3) UI infrastructure types from third-party crates (e.g. `bevy_lunex` layout/rendering types in domain crates)
